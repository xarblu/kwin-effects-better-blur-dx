...
BlurEffect::BlurEffect()
{
    ...
    if (!m_contrastPass.shader) {
        qCWarning(KWIN_BLUR) << BBDX::LOG_PREFIX << "Failed to load contrast pass shader";
        return;
    } else {
...
bool BlurEffect::shouldBlur(const EffectWindow *w, int mask, const WindowPaintData &data)
{
    ...
    if ((scaled || (translated || (mask & PAINT_WINDOW_TRANSFORMED))) && !w->data(WindowForceBlurRole).toBool() && !m_windowManager.windowForceBlurred(w)) {
        return false;
    }
